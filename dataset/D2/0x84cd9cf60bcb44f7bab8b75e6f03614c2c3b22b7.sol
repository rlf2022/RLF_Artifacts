...
contract ESmart {
    ...
    function pay() private {
        ...
        for(uint i=currentReceiverIndex; i= dep.expect){  //If we have enough money on the contract to fully pay to investor
    ...
    function getDepositsCount(address depositor) public view returns (uint) {
        ...
        for(uint i=currentReceiverIndex; i 0) {
            uint j = 0;
            for(uint i=currentReceiverIndex; i 0 && lastDepositInfo.time + MAX_IDLE_TIME <= now){
